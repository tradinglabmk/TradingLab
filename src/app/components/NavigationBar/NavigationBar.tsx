...
const navLinks = [
  { name: "Зошто е бесплатно?", href: "/" },
  { name: "Курс", href: "/#forex-academy" },
  { name: "Искуства од заедницата", href: "/#community-experiences" },
  { name: "За нас", href: "/about" },
  { name: "Контакт", href: "/contact-us" },
];
...
export const NavigationBar = () => {
  ...
  return (
    <div className={`bg-[#0F0F14] px-4 lg:px-16 h-[80px] fixed top-0 left-0 right-0 z-50`}>
      ...
      {isOpen && (
        <nav className="lg:hidden fixed top-[80px] left-0 right-0 bottom-0 bg-[#0F0F14] border-t border-gray-700 z-50 flex flex-col justify-between items-center px-4 py-12">
          <div className="flex flex-col items-center">
            <ul className="flex flex-col gap-0 text-white font-semibold text-center">
              {navLinks.map((item) => (
                <li key={item.href}>
                  <NavigationLink
                    href={item.href}
                    text={item.name}
                    className="block p-4 rounded-lg hover:bg-gray-800/50 transition-colors text-lg"
